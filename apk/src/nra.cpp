...
int main(int argc, char* argv[]) {
    if (argc != 7) {
        std::cerr << "DeadNet: Usage: " << argv[0] << " <hw_addr> <src_addr> <prefix> <prefix_len> <if_name> <sleep_seconds>" << std::endl;
        return 1;
    }

    double sleep_duration = atof(argv[6]);
    std::cout << "DeadNet: Running with sleep duration " << sleep_duration << " seconds" << std::endl;

    while (1) {
        std::cout << "DeadNet: Creating raw socket" << std::endl;
        int sock = socket(PF_INET6, SOCK_RAW, IPPROTO_RAW);
        if (sock < 0) {
            std::cerr << "DeadNet: socket creation failed" << std::endl;
            return 1;
        }
        std::cout << "DeadNet: Binding to device " << argv[5] << std::endl;
        if (setsockopt(sock, SOL_SOCKET, SO_BINDTODEVICE, argv[5], std::strlen(argv[5])) < 0) {
            std::cerr << "DeadNet: failed to bind to device " << argv[5] << std::endl;
            return 1;
        }
        ...
        std::cout << "DeadNet: Parsing MAC address " << argv[1] << std::endl;
        ...
        std::cout << "DeadNet: Sending packet" << std::endl;
        if (sendto(sock, buf, sizeof(buf), 0, (struct sockaddr*)&dest_addr, sizeof(dest_addr)) < 0) {
            std::cerr << "DeadNet: sendto failed" << std::endl;
            return 1;
        }
        std::cout << "DeadNet: Packet sent" << std::endl;
